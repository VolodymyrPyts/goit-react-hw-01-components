...
return <Box width="300px" mt='15px' backgroundColor="white" ml='auto' mr='auto'  borderRadius='10px' className="profile" >
  ...
  <Box position='center'  p='20px' className="description">
    <ImgUser
      ...
      className="avatar"
    />
    <NameUser className="name">{user.username}</NameUser>
    <InformUser className="tag">@{user.tag}</InformUser>
    <InformUser className="location">{user.location}</InformUser>
  </Box>

  <UserList className="stats">
    <UserItems>
      <UserLable className="label">Followers</UserLable>
      <UserQuantity className="quantity">{user.stats.followers}</UserQuantity>
    </UserItems>
    <UserItems>
      <UserLable className="label">Views</UserLable>
      <UserQuantity className="quantity">{user.stats.views}</UserQuantity>
    </UserItems>
    <UserItems>
      <UserLable className="label">Likes</UserLable>
      <UserQuantity className="quantity">{user.stats.likes}</UserQuantity>
    </UserItems>
  </UserList>
</Box>
}
